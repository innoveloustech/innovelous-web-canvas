...
export default function AdminDashboardPortal() {
  ...
  const dismissModalContext = () => {
    if (modalWrapperRef.current && modalBoxRef.current) {
      ...
      tl.to(modalBoxRef.current, { scale: 0.95, y: 10, opacity: 0, duration: 0.2, ease: "power2.in" }).to(modalWrapperRef.current, { opacity: 0, duration: 0.15 }, "-=0.1");
    } else {
      setModalMode(null);
    }
  };
  ...
  const clearFormFields = () => {
    setTitle(""); setCategory("Web Development"); setDescription(""); setLink(""); setColor("#a855f7"); setMediaFile(null);
  };

  const openFormModal = (mode: "CREATE" | "UPDATE" | "DELETE", project?: Project) => {
    if (project) {
      setActiveProject(project); setTitle(project.title); setCategory(project.category); setDescription(project.description); setLink(project.link); setColor(project.color || "#a855f7");
    } else {
      clearFormFields();
    }
    setModalMode(mode);
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0a0a0a] text-white px-4 font-sans">
        <div className="w-full max-w-md border border-white/10 bg-white/[0.02] backdrop-blur-md p-8 rounded-3xl">
          ...
          <form onSubmit={executeLogin} className="space-y-4">
            <input type="email" placeholder="Identifier Email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} required className="w-full p-4 bg-black border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:border-purple-500 transition-colors" />
            <input type="password" placeholder="Key Verification Token" value={authPassword} onChange={e => setAuthPassword(e.target.value)} required className="w-full p-4 bg-black border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:border-purple-500 transition-colors" />
            <button type="submit" className="w-full p-4 bg-white text-black text-xs uppercase tracking-wider font-bold rounded-xl hover:bg-neutral-200 transition-colors">Connect Workspace</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pt-24 pb-12 px-6 md:px-16 font-sans">
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/10 pb-8 mb-12 gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-tight">Dashboard To Managae Projects</h1>
        ...
        <div className="flex gap-3">
          <button onClick={() => openFormModal("CREATE")} className="px-5 py-3 bg-purple-600 hover:bg-purple-700 text-white font-medium text-xs uppercase tracking-wider rounded-xl transition-colors">Create Object</button>
          <button onClick={() => supabase.auth.signOut()} className="px-5 py-3 border border-white/10 hover:bg-white/5 text-neutral-400 hover:text-white text-xs uppercase tracking-wider rounded-xl transition-colors">Log Out</button>
        </div>
      </div>
          ...
          <div key={project.id} className="group border border-white/10 bg-white/[0.01] rounded-2xl p-6 flex flex-col justify-between hover:bg-white/[0.03] transition-colors min-h-[260px]">
            <div>
              <div className="flex justify-between items-start gap-4 mb-4">
                <span className="text-[9px] font-mono tracking-widest uppercase px-2 py-0.5 border rounded-full" style={{ color: project.color, borderColor: project.color }}>{project.category}</span>
                {project.image_url && <div className="w-12 h-12 rounded-lg border border-white/10 overflow-hidden bg-black flex-shrink-0"><img src={project.image_url} alt="asset node preview" className="w-full h-full object-cover" /></div>}
              </div>
            ...
            <div className="flex gap-2 border-t border-white/5 pt-4 mt-6">
              <button onClick={() => openFormModal("UPDATE", project)} className="flex-1 py-2 text-center text-xs border border-white/10 rounded-lg text-neutral-300 hover:bg-white/5 transition-colors">Modify</button>
              <button onClick={() => openFormModal("DELETE", project)} className="px-3 py-2 text-center text-xs border border-red-500/20 rounded-lg text-red-400 hover:bg-red-500/10 transition-colors">Delete</button>
            </div>
          </div>
        ))}
      </div>
          ...
          <div ref={modalBoxRef} className="w-full max-w-xl bg-[#0f0f11] border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            {modalMode === "DELETE" ? (
              <div>
                ...
                <p className="text-neutral-400 text-sm font-light leading-relaxed mb-6">Are you sure you want to permanently delete <span className="text-white font-medium">{activeProject?.title}</span>? This instruction will drop the values from postgres servers instantly.</p>
                <div className="flex gap-3 justify-end">
                  ...
                  <button onClick={handleDeletionCall} disabled={processing} className="px-5 py-2.5 text-xs uppercase tracking-wider font-semibold bg-red-600 hover:bg-red-700 text-white rounded-xl disabled:bg-neutral-800 transition-colors">{processing ? "Purging..." : "Confirm Delete"}</button>
                </div>
              </div>
            ) : (
              <div>
                <h3 className="text-2xl font-light tracking-tight text-white mb-6">{modalMode === "CREATE" ? "Initialize Data Object" : "Modify Structural Values"}</h3>
                <form onSubmit={handleFormSubmission} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="p-4 border border-dashed border-white/10 rounded-xl text-center relative hover:border-white/20 transition-colors">
                      ...
                      <p className="text-xs text-neutral-400 font-light">{mediaFile ? `Queued: ${mediaFile.name}` : modalMode === "UPDATE" ? "Leave blank to keep current thumbnail asset" : "Click or drop thumbnail image asset package here"}</p>
                    </div>
                  </div>
