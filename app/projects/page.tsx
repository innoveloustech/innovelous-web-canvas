...
export default function ProjectsPage() {
  ...
  // Option 3 Hover Interactions for the Modal Thumbnail Preview
  ...
  return (
    <>
      ...
      <main ref={containerRef} className="relative min-h-screen bg-[#0a0a0a] text-white pt-32 pb-20 px-6 md:px-16 overflow-hidden">
        ...
        {loading ? (
          <div className="max-w-7xl mx-auto flex items-center justify-center py-20">
            <p className="font-mono text-xs uppercase tracking-widest text-neutral-500 animate-pulse">Syncing production nodes...</p>
          </div>
        ) : projects.length === 0 ? (
          <div className="max-w-7xl mx-auto flex items-center justify-center py-20 border border-dashed border-white/5 rounded-3xl">
            <p className="font-mono text-xs uppercase tracking-widest text-neutral-600">No active systems initialized.</p>
          </div>
        ) : (
      ...
      {selectedProject && (
        <div ref={modalRef} className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-black/80 backdrop-blur-md" onClick={closeModal}>
          <div ref={modalContentRef} className="relative w-full max-w-5xl bg-[#0f0f11] border border-white/10 rounded-3xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeModal} className="absolute top-6 right-6 z-20 w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 hover:border-white/20 transition-all duration-200">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <div className="grid grid-cols-1 lg:grid-cols-2">
              {/* Option 3 Left Side: High Resolution Image Preview Window with Scale Transition */}
              <div 
                className="relative h-64 lg:h-auto min-h-[440px] bg-neutral-900 overflow-hidden cursor-pointer group"
              ...
              >
                {selectedProject.image_url ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-neutral-600 font-mono text-xs uppercase tracking-widest">No Asset Loaded</div>
                )}
              ...
              <div className="p-8 md:p-12 flex flex-col justify-center bg-[#0f0f11]">
                <span className="text-[10px] font-mono tracking-[0.25em] uppercase mb-4 block" style={{ color: selectedProject.color }}>{selectedProject.category}</span>
                <h2 className="text-3xl md:text-5xl font-black tracking-tighter text-white mb-6">{selectedProject.title}</h2>
                <p className="text-neutral-400 text-base leading-relaxed font-light mb-12">{selectedProject.description}</p>
                <div className="flex flex-col sm:flex-row gap-4">
                  {selectedProject.link && (
                    <a href={selectedProject.link} target="_blank" rel="noopener noreferrer" data-cursor="-exclusion" className="group relative flex items-center justify-center gap-3 bg-white text-black px-8 py-4 rounded-full font-semibold text-sm overflow-hidden transition-transform hover:scale-105">
                      ...
                      <svg className="relative z-10 w-4 h-4 text-black group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                    </a>
                  )}
                  <button onClick={closeModal} className="flex items-center justify-center gap-2 px-8 py-4 rounded-full border border-white/10 text-white text-sm font-medium hover:bg-white/5 transition-colors duration-300">Close</button>
                </div>
              </div>
            </div>
          </div>
